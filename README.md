...
- 公开上号页固定在 `/oplogin`
- 短 OP 输入页固定在 `/op`，带短码的分享链接使用 `/op/:code`
- `POST /api/submit` 优先本地生成唤起链接，失败时自动回退远程接口
- 管理后台支持登录、记录管理、短 OP 管理、应用管理、批量导入、CSV 导出
...
| 角色 | 能力 |
| --- | --- |
| `super_admin` | 登录后台、管理全部记录、短 OP 和应用，管理后台账号、重置密码、维护二维码配置、启用/禁用用户 |
| `operator` | 登录后台、查看和操作自己权限范围内的数据及短 OP；不能进入账号管理，后台不显示“应用管理”入口 |
...
- 短 OP 输入页：<http://localhost:4399/op>
...
- `DELETE /api/admin/short-ops/:id` 执行软删除；8 位短码在全表永久唯一，删除后永不重新分配。
