...
const PHONE_DEFAULT_DURATION_MS = 30 * 24 * 60 * 60 * 1000;

function derivePhoneExpireAt(now = Date.now()) {
  return new Date(now + PHONE_DEFAULT_DURATION_MS).toISOString();
}
      ...
      select
        ...
        coalesce(sum(case
          when google_account != ''
            and google_password_search_hash != $1
            and op_value != ''
            and (uid_value = '' or uid_value is null) then 1
          else 0
    ...
    if (parts.length === 2) {
      ...
      return {
        ...
        data: {
          ...
          phoneExpireAt: derivePhoneExpireAt(),
          phoneStatus: '未绑定',
          phoneModel: '12mini',
          opValue: '',
          opLink: '',
