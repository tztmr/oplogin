    ...
    ? await client.query(
      `
        ...
        where m.owner_id = $1
          ...
          and m.op_value != ''
          and m.google_password_search_hash != $3
...
function hasProcessableSlots(batch) {
  return batch.slots.some((slot) => slot.status === 'available' && slot.record);
}
...
async function getCurrentBatch(pool, config, user) {
  ...
  try {
    ...
    if (
      openBatchId &&
      !hasProcessableSlots(batch) &&
      await hasEligibleRecords(client, config, user.id)
    ) {
      await client.query(
        `
...
async function advanceBatch(pool, config, user) {
  ...
  try {
    ...
    const priorityRecordIds = releasableResult.rows.map((row) => row.record_id);
