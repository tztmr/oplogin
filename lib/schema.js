const { migrateLegacyPhones } = require('./phone-inventory-migration');
...
async function ensureDatabaseSchema(pool) {
  await pool.query(`
    ...
    create table if not exists phone_inventory_legacy_archive (
      id uuid primary key,
      ...
      phone_connected text not null,
      phone_status text not null,
      source_created_at timestamptz not null,
      source_updated_at timestamptz not null,
      archive_reason text not null,
      archived_at timestamptz not null default now(),
      unique (migration_name, source_record_id)
    );
    ...
    alter table managed_records add column if not exists phone_number text not null default '';
    alter table managed_records add column if not exists phone_sms_url text not null default '';
    alter table managed_records add column if not exists phone_expire_at timestamptz null;
    alter table managed_records add column if not exists phone_connected text not null default '未连接';
    alter table managed_records add column if not exists phone_status text not null default '未绑定';
    alter table managed_records add column if not exists phone_model text not null default '12mini';
    ...
    create index if not exists idx_phone_inventory_owner_record
      on phone_inventory (owner_id, reserved_record_id);
    ...
    create index if not exists idx_phone_inventory_archive_source_record
      on phone_inventory_legacy_archive (source_record_id, phone_status);
