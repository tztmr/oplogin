...
async function createShortOpRecord(pool, payload, adminUser, options = {}) {
  ...
  try {
    ...
    throw createHttpError(409, '短码生成冲突，请重试');
  } catch (error) {
    if (!completed) {
      await client.query('rollback');
    }
    if (error.code === '23505') {
      throw createHttpError(409, '该 OP 与应用已存在');
    }
    throw error;
  } finally {
    client.release();
...
async function getShortOpRecordById(pool, id, adminUser) {
  const row = await queryShortOpById(pool, id, adminUser);
...
async function updateShortOpRecord(pool, id, payload, adminUser) {
  ...
  try {
    await client.query('begin');
    const existing = await queryShortOpById(client, id, adminUser);
    if (!existing) {
      await client.query('rollback');
      completed = true;
      return null;
    ...
    const application = await lockActiveApplication(client, input.applicationId);
    const values = [
      id, input.opValue, input.applicationId, input.opExpireAt, input.remark,
    ];
    let ownerSql = '';
...
async function setShortOpRecordStatus(pool, id, status, adminUser) {
  ...
  return getShortOpRecordById(pool, id, adminUser);
}
...
async function deleteShortOpRecord(pool, id, adminUser) {
  ...
  if (!result.rows[0]) return null;
  const row = await queryShortOpById(pool, id, adminUser, { includeDeleted: true });
  return row ? toShortOpDto(row, { includeOpValue: true }) : null;
}
...
async function importShortOpText(pool, rowsText, adminUser, options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      try {
        items.push(await createShortOpRecord(pool, {
          opValue: parsed.opValue,
          applicationId: application.id,
        }, adminUser, options));
      } catch (error) {
        if (error.statusCode === 409 && error.message === '该 OP 与应用已存在') {
          duplicateCount += 1;
          continue;
        }
        throw error;
